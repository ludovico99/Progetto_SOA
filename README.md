...
2.  uint64_t file_size = filp->f_inode->i_size è la dimensione complessiva, considerando sia i blocchi validi che invalidi, del device file. Il valore precedente non può cambiare a runtime, è definita a tempo di compilazione. Nel mio caso quindi, non rappresenta la dimensione attuale del file (soli blocchi validi), che è mutevole in quanto altri threads attraverso operazioni di put_data e invalidate_data possono alterare. Ciò nonostante,  si è deciso di salvare il valore di file_size in un campo di una struttura globale nel momento in cui si va a leggere l'inode del file mantenuto sul device (vedere [qui](/user_data_management_project/userdatamgmt_fs_src.c#L58)).
...
3. Il blocco da leggere nella dev_read, in precedenza, veniva calcolato come block_to_read = my_off / BLK_SIZE + 2. In questo caso l'offset indentificava un byte in un blocco e non un byte nell'effettivo messaggio (payload). I metadati di fatto non sono dati leggibili e di conseguenza non vanno considerati nell'individuazione del blocco da leggere. Questo problema è stato risolto nei seguenti 3 passaggi:
    1. Il blocco da leggere è block_to_read = get_offset(curr_msg->index). Il nuovo blocco da leggere è individuato dall'offset dell'indice del blocco corrente (blocco al quale si vuole accedere in lettura) (vedere [qui](/user_data_management_project/userdatamgmt_driver.c#L104))
    2. Il nuovo offset, per un nuovo blocco da leggere, all'interno del device file è un multiplo di SIZE e non piu di BLK_SIZE. Notare che in questa variante l'offset non viene utilizzato per calcolare l'indice del blocco da leggere, bensi è salvato nella sessione di I/O per valutare possibili residui nella lettura del blocco corrente in dev_read successive. (vedere [qui](/user_data_management_project/userdatamgmt_driver.c#L100))
    ...
    Di conseguenza, l'offset del blocco da leggere è calcolato come indice del blocco corrente + 2 e non in base alla dimensione complessiva del file.

4. All’interno della sys_get_data andavo erroneamente, in un while, a controllare il valore restituito dalla copy_to_user. Questo perché volevo provare a gestire eventuali residui nella lettura. Ovviamente questa situazione andrebbe gestita lato utente. La presenza di una while(!condition) nel kernel, come da Lei spiegato durante il corso, ha portato il thread a non poter rilasciare la CPU a vantaggio di qualcun altro. (vedere [qui](/user_data_management_project/userdatamgmt_sc.c#L291))