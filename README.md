...
- [userdatamgmt_driver.h](/user_data_management_project/userdatamgmt_driver.c): contiene le definizioni delle strutture dati utilizzate per la realizzazione del Block-level data management service. L'approfondimento sulle data structures utilizzate è nella [sezione precedente](/README.md#strutture-dati)
...
- [utils.h](/user_data_management_project/utils.h): contiene i prototipi delle funzioni "ausiliarie" presenti in utils.c. Sono presenti i seguenti prototipi:
    - void [init](/user_data_management_project/utils.c#L56)(struct rcu_data *): funzione di inizializzazione dei campi della struttura RCU
    - void [insert](/user_data_management_project/utils.c#L91)(struct message **, struct message **, struct message *): inserimento in coda del messaggio in input
    ...
    - void [free_list](/user_data_management_project/utils.c#L170)(struct message *): funzione per liberare la lista dei messaggi validi
    - void [free_array](/user_data_management_project/utils.c#L189)(struct blk_element **): funzione per eseguire la free sull'array di metadati
    - void [delete](/user_data_management_project/utils.c#L213)(struct message **, struct message **, struct message *): funzione per eliminare il messaggio in input dalla lista dei messaggi validi
    - struct message *[lookup_by_insert_index](/user_data_management_project/utils.c#L286)(struct message *, int): lookup del messaggio in base al valore del campo insert_index
    - struct message *[lookup_by_index](/user_data_management_project/utils.c#L302)(struct message *, int): lookup del messaggio in base al suo indice
    - void [quickSort](/user_data_management_project/utils.c#L374)(struct message *, struct message *): funzione che implementa il quick sort 
    - void [print_list](/user_data_management_project/utils.c#L256)(struct message *): funzione di debugging per stampare la lista dei messaggi validi a partire dalla testa
    - void [print_reverse](/user_data_management_project/utils.c#L271)(struct message *): funzione di debugging per stampare la lista dei messaggi validi a partire dalla coda
...
- struct dentry * [userdatafs_mount](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L187)(struct file_system_type *fs_type, int flags, const char *dev_name, void *data): è la funzione che viene invocata nella fase di mounting del file system. Può essere sintetizzata nei seguenti step:
    ...
    3. Se la mount_bdev ha avuto **successo**:
        1. Viene inizializzata la **struttura dati condivisa RCU** . [Vedere qui](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L230)
        2. Viene allocato un **array di size nblocks**  (numero di blocchi totali - 2 del device) che conterrà i metadati di ogni blocco. In base al numero di bytes da allocare si sceglie di utilizzare kzalloc () o vmalloc(). [Vedere qui](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L232)
        3. Per ogni i da 0 a nblock - 1  ([Vedere qui](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L245)):
    ...
    4. Si va a settare la variabile globale numero di inserimenti pari alla posizione dell'ultimo elemento all'interno della lista dei messaggi validi. La variabile suddetta indica il numero di totale di messaggi inseriti all'interno della lista doppiamente collegata. È l'indice di partenza per numerare/ indicizzare le successive sys_put_data. [Vedere qui](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L304)

    5. Si controlla se c'è stato qualche **errore** durante l'esecuzione. In questo caso **mounted viene riportato a 0** poichè la mount non ha avuto successo. In questo modo, se la mount dovesse fallire, è possibile rieseguirla. [Vedere qui](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L318)

>**NOTE**: L'operazione di mount in questo caso ha costo O(n^2): per ogni blocco da leggere è richiesto un inserimento sorted. Per evitare questo problema è presente del [codice](/user_data_management_project/utils.c#L376), in utils.c, che realizza il **quick sort** a partire da una lista di elementi non ordinata. In questo modo il costo è O(n*logn). Il problema in questo caso è che il quick sort è implementato in modo ricorsivo. Di conseguenza, in presenza di un numero elevato di blocchi validi, oppure nel caso degenere di un array già ordinato (il pivot è scelto come ultimo elemento della partizione), lo stack di livello kernel potrebbe esaurirsi. Ovviamente, questo è un tradeoff tra prestazioni e utilizzo di memoria. Attualmente la versione con il quick sort è commentata.

- static void [userdatafs_kill_superblock](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L102) (struct super_block *s): è la funzione che viene invocata nella fase di unmounting del file system. È caratterizzata dai seguenti passi:
    1. Come sopra, per prima cosa, un **solo thread porta a 0** il valore dell'intero mounted. In questo modo è possibile fare nuovamente il mounting in futuro. [Vedere qui](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L117)
    2. Si va in wait, secondo un approccio basato su **polling**, fintanto che l'usage counter (il numero di thread che stanno correntemente lavorando sul device) è diverso zero.  [Vedere qui](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L128)
    3. Una volta diventato zero, si itera sugli elementi della lista di messaggi validi e si realizza il **filling dei gaps del campo position**, a causa di operazioni di invalidate, in modo da evitare l'**overflow**. Il valore di position altrimenti, sarebbe sempre aumentato in successive mount e unmount del FS. In questa fase non ho necessità di mantenere all'interno del messaggio il suo valore di insert_index, cioè di memorizzare l'indice della sys_put_data che ha scritto quel blocco.[Vedere qui](/user_data_management_project/file_system/userdatamgmt_fs_src.c#L132)
...
- int [sys_put_data](/user_data_management_project/userdatamgmt_sc.c#L4)(char* source, size_t size): Ha l'obiettivo di andare ad inserire sul device size bytes all'interno del buffer source in un blocco invalido. Si è deciso di "mantenere" l'ordine di inserimento andando a scrivere in coda alla lista dei messaggi validi. L'algoritmo prevede i seguenti passi:
    ...
    10. Se l'operazione **ha avuto successo** si procede con l'aggiornamento delle strutture dati condivise: 
        ...
        2. Si fa puntare l'elemento dell'array contenente i metadati al nuovo messaggio e si aggiornano i metadati. [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L162)
        ...
        3. Infine, si inserisce l'elemento all'interno della lista doppiamente collegata. [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L166)
...
- int [sys_get_data](/user_data_management_project/userdatamgmt_sc.c#L226) (int offset, char * destination, size_t size): Ha l'obiettivo di andare a leggere size bytes del blocco ad indice offset e riportarne il contenuto nel buffer destination lato user.
    ...
    [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L315)
...
- int [sys_invalidate_data](/user_data_management_project/userdatamgmt_sc.c#L331) (int offset):
    1. Si rende impossibile l'operazione di unmount del FS aggiungendo 1 all'usage counter. [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L354)
    2. Si verifica che il file system sia stato montato. [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L357)
    3. Si controlla il valore di offset: se offset è minore di 0 o maggiore di nblocks -1 viene ritornato -EINVAL.  [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L364)
    4.  Il thread corrente **va in attesa del lock in scrittura**. [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L371).
    5. Una volta **acquisito il lock**, si accede con costo 0(1) ai metadati del blocco ad indice pari all'offset in input. Se il blocco è già stato invalidato allora viene ritornato -ENODATA. [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L376)
    6. Si procede ad **aggiornare la lista dei messaggi validi**, andando ad eliminare l'elemento, se esiste, con indice pari ad offset dalla lista. Ciò, è fatto con costo costante O(1) in seguito all'utilizzo di una lista doppiamente collegata ([Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L393)). 
    ...
    7. Viene individuata la nuova epoca nel seguente modo:
        ...
        [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L405)

    8. Al termine dell'epoca viene rilasciato lo spinlock, viene liberata la memoria del buffer, si decrementa l'usage counter e si risveglia il thread in attesa quando si verifica la condizione desiderata. [Vedere qui](/user_data_management_project/userdatamgmt_sc.c#L421)
...
- int [open](/user_data_management_project/userdatamgmt_driver.c#L182) (struct inode*, struct file*): 
    1. Innanzitutto, si verifica che il file system sia stato montato. [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L194)
    2. Se la modalità di apertura del file è in write mode, si ritorna il codice d'errore (Read-only FS). [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L199)
    3. Si rilascia un **gettone sull'usage counter globale**. [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L204)
    ...
    Il campo offset è posto a 0, insert_index a -1 e curr a NULL. [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L206)

- int [release](/user_data_management_project/userdatamgmt_driver.c#L159)(struct inode*, struct file*):
    1. Innanzitutto, si verifica che il file system sia stato montato. [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L170)
    2. Si **libera la struct current_message dell'attuale sessione di I/O**. [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L176)
    3. Si **decrementa l'usage counter globale** e si risveglia il thread in attesa quando si verifica la condizione desiderata. [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L177)
...
- ssize_t [read](/user_data_management_project/userdatamgmt_driver.c#L1) (struct file*, char __user*, size_t, loff_t*):
    ...
    5. Si verifica se l'offset sia un multiplo di BLK_SIZE. Se il resto della divisione con BLK_SIZE è diverso da 0, significa che **ci sono dei bytes del blocco precedente che non sono stati consegnati al lettore**. Di conseguenza, a meno che, in concorrenza, tra una lettura e la successiva il blocco sia stato invalidato, si continua a leggere il blocco corrente. [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L49)
    ...
    **Se entrambe le condizioni sono vere significa che lo scrittore ha rilasciato il buffer**. Se la condizione è verificata si va a leggere il blocco successivo.
    ...
    6. Se è la prima lettura, cioè è successiva alla open, allora **il messaggio corrente è il primo elemento della lista dei messaggi validi**. Altrimenti il prossimo messaggio è quello **il cui indice d'inserimento è almeno successivo a quello dell'elemento letto in precedenza**. In quest'ultimo caso si fa una ricerca lineare nella lista e si ritorno il blocco il cui insert_index è maggiore o uguale di quella in input (individuata nella lettura precedente). [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L65)
    7. Una volta individuato il blocco da leggere, si calcola l'offset e si accede al device. [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L87). 
    8. Si va a leggere un numero di bytes pari alla lunghezza del messaggio all'interno del blocco, se len è maggiore di str_len. Altrimenti, si leggono len bytes. Infine, si consegnano all'utente attraverso copy_to_user(). [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#121).
    9. Se **copy_to_user () ritorna zero** allora ho letto tutti i bytes del messaggio nel blocco. Si andrà a **leggere il blocco successivo**. Se **ritorna un valore diverso da zero** allora nella read successiva si **andrà a rileggere lo stesso blocco**, se non invalidato. 

        Nel primo caso si andrà ad inviduare, nella lettura successiva, il messaggio valido (Anche un eventuale inserimento linearizzato di un messaggio attravero sys_put_data), il cui insert_index è maggiore o uguale di quella del blocco letto + 1. Di conseguenza, se il messaggio successivo venisse invalidato prima di iniziare la lettura corrispondente, la funzione [lookup_by_insert_index](/user_data_management_project/userdatamgmt_driver.c#L75) andrà a trovare l'elemento seguente (se esiste) a quello invalidato, che avrà un valore per l'indice di inserimento almeno uguale a quella corrente + 1. Di conseguenza, verrà comunque letto il blocco corretto dal punto di vista della concorrenza.
    ...
    [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L135)

    10. Infine, viene individuato, a partire dall'epoca corrente, l'indice all'interno dell'array standing. Viene, quindi, aggiunto 1 nell'entry con l'index trovato. [Vedere qui](/user_data_management_project/userdatamgmt_driver.c#L148)
...
- int [sys_get_data](/user_data_management_project/userdatamgmt_sc.c#L224) (int offset, char * destination, size_t size): Nella sys_get_data, come sopra, la struttura dati condivisa con i messaggi validi è **acceduta unidirezionalmente** a partire dalla testa. **Anche in questo caso non si accede all'array dei metadati**. Così facendo la linearizzazione può essere raggiunta.
...
- int [sys_invalidate_data](/user_data_management_project/userdatamgmt_sc.c#L329) (int offset): Sappiamo che l'invalidate_data, invece, va a sganciare un elemento all'interno della lista dei messaggi validi.                     
...
- [user.c](/user_data_management_project/user/user.c): è il codice di prova, sviluppato per testare le system calls e la concorrenza tra scrittori e lettori, anche sullo stesso blocco. All'interno del Makefile sono previste le seguenti **modalità d'esecuzione**:
    ...
    - [get_data](/user_data_management_project/user/Makefile#L25): Vengono acquisiti i dati del blocco, se valido, con l'indice (offset nel blocco - 2) passato come argomento nel caso single thread. Nel caso multi-thread si comporta come gets.
    ...
    - [invalidations](/user_data_management_project/user/Makefile#L32): Vengono invalidati (da uno dei thread creati) tutti i blocchi, se non sono già stati invalidati, passati come argomento.
